...
-- Insert unique countries from the staging customers table
-- country_id is automatically generated by AUTOINCREMENT
...
-- Insert unique states and map each state to its corresponding country_id
-- country_id is retrieved by joining with the country dimension
...
-- Insert unique cities and map each city to its corresponding state_id
-- state_id is retrieved by joining with the state dimension
...
-- Insert customer records and map each customer to its corresponding city_id
-- city_id is retrieved by joining with the city dimension
